...
function generateSummaryText(diff) {
    ...
    // Section renames — displayed on a separate line
    let sectionLine = '';
    const sectionRenames = diff.groups.filter(g => g.oldStateName).map(g => `${g.oldStateName} → ${g.stateName}`);
    if (sectionRenames.length) {
        sectionLine = `<div class="summary-text-chain"><span class="hi">section rename${sectionRenames.length === 1 ? '' : 's'}:</span> ${sectionRenames.map(s => `<code>${esc(s)}</code>`).join(', ')}</div>`;
    }
    ...
    el.innerHTML = parts.join('<span style="color:var(--border);margin:0 6px">·</span>') + sectionLine + chainLine;
    el.style.display = (parts.length || sectionLine || chainLine) ? 'block' : 'none';
